...
**測試時間：** 2026-05-27
**測試檔案：** `L9534589_K52600039.pdf`（彰化院外 EGFR cobas 報告，3 頁）

### 結果

| 項目 | 結果 |
|------|------|
| pdfplumber 文字擷取 | 可抽出文字（7,253 字） |
| 中文欄位（姓名、科別等） | 嚴重失真，大量重複字元（如「二二二二二二二」、「許許許」） |
| 英文欄位（EGFR mutation 報告本文） | 正確擷取 |
| LLM 解析英文變異 | 正確找到 EGFR Ex19Del、T790M |
| LLM 解析中文 metadata | 姓名/日期等因字型失真而解析不穩定 |

### 根本原因

院內 PDF 使用 CID 字型（Type 0/CIDFont），pdfplumber 對這類字型的中文 unicode mapping 無法保證正確，導致字元重複或亂碼。此問題沒有規律，不同 PDF 生成工具產出的結果差異極大，無法用後處理修正。

OCR 方案（pytesseract 等）可解決，但需要額外維護，且準確率仍受掃描品質影響，本公司目前無資源投入。

### 結論：不建議以 PDF 作為資料交換格式

建議由院方與外部基因檢測機構協調，要求對方直接提交純文字報告（TXT）或 VCF 檔案。目前已有醫院以此要求外部單位，且外部單位通常可配合。

- **TXT**：適合有敘述性報告的機構（如 NGS panel 報告），LLM 解析效果佳
- **VCF**：適合全基因組/外顯子定序，直接走 `vcf_to_fhir` 轉換，無需 LLM
